...
// 2. Go to the Domestic Travel page
// 3. Select city, filter products
// 4. Go to the details page
test('DomesticTravel', async ({}) => {
    const departureCity = "北海";
    ...
    //Go to the Domestic Travel page
    await page.locator('#vac-103045-recommend-more-境内旅游-2-更多精选产品').click();
    ...
    try {
        await expect(page1.locator('#filter_box_point > div:nth-child(2) > div.list_cate_content.basefix > div.list_cate_left > div > div').getByText(departureCity, { exact: true })).toBeVisible();
        await page1.locator('#filter_box_point > div:nth-child(2) > div.list_cate_content.basefix > div.list_cate_left > div > div').getByText(departureCity, { exact: true }).click();
